...
from tuning import has_ranked_examples, render_ranked_examples, render_tuning
...
def parse_rich_docs() -> dict[str, RichDocs]:
    ...
    rich = {}
    docs = sorted(GLOSSARY_RS.with_suffix("").glob("docs/*.rs"))
...
def render_findings() -> str:
    ...
    out = [
        "# Finding Reference\n\n",
        "This page is generated by `docs/generate.py` from `src/noze/glossary/`.\n\n",
        '<div id="sensez-findings-tabs" hidden></div>\n\n',
    ]
    rendered_examples = {kind: render_example_and_fix_tabs(doc) for kind, doc in rich.items()}
    for group_title, group in grouped_smells(smells):
        out.append(f"## {group_title}\n\n")
        for smell in group:
            doc = rich[smell.kind]
            ranked = has_ranked_examples(smell.term)
            examples = (
                render_ranked_examples(smell.term, doc.fixes)
                if ranked
                else rendered_examples[smell.kind]
            )
            out.extend(
                [
